...
void suite_one()
{
    ...
    CHECK_EQUAL( memcmp( buffer, "1\0003-5+7\177", 8 ), 0 );
    ...
    CHECK_EQUAL( memcmp( buffer, "\377\177", 2 ), 0 );
    ...
    CHECK_EQUAL( memcmp( buffer, "\011\177", 2 ), 0 );
    ...
    CHECK_EQUAL( memcmp( buffer, "0\177", 2 ), 0 );
    CHECK_EQUAL( feof( file ), 1 );
    CHECK_EQUAL( ferror( file ), 0 );
    CHECK_EQUAL( ftell( file ), 40 );
    }
    ...
    CHECK_EQUAL( fscanf( file, "%1c", buffer ), 1 );
    CHECK_EQUAL( memcmp( buffer, "0\177", 2 ), 0 );
    CHECK_EQUAL( feof( file ), 0 );
    CHECK_EQUAL( ftell( file ), 40 );
    }
    {
    // reading a char array of specified width 1 when positioned at EOF
    // TODO: could not figure out what the spec is in this case, results given match Linux Mint
    fseek( file, 0, SEEK_END );
    char buffer[ 2 ];
    memset( buffer, '\177', 2 );
    CHECK_EQUAL( fscanf( file, "%1c", buffer ), -1 );
    CHECK_EQUAL( memcmp( buffer, "\177\177", 2 ), 0 );
    CHECK_EQUAL( feof( file ), 1 );
    CHECK_EQUAL( ferror( file ), 0 );
    CHECK_EQUAL( ftell( file ), 40 );
    }
    {
    // reading a char array of width zero
    // TODO: could not figure out what the spec is in this case, results given match Linux Mint
    // FIXME: Not agreeing with this result; %0c should read zero characters
    fseek( file, 0, SEEK_SET );
    char buffer[ 2 ];
    memset( buffer, '\177', 2 );
    CHECK_EQUAL( fscanf( file, "%0c", buffer ), 1 );
    CHECK_EQUAL( memcmp( buffer, "1\177", 2 ), 0 );
    CHECK_EQUAL( ftell( file ), 1 );
    }
    {
    // reading a char array of width zero when positioned at EOF
    // TODO: could not figure out what the spec is in this case, results given match Linux Mint
    fseek( file, 0, SEEK_END );
    char buffer[ 2 ];
    memset( buffer, '\177', 2 );
...
void suite_three()
{
    ...
    int n;
    CHECK_EQUAL( fscanf( file, "%hhx%n", &i, &n ), 1 );
    CHECK_FEQUAL( i, -127, unsigned char, "%hhu" ); // FIXME: This reports as 129... huh?!?
    CHECK_EQUAL( n, 5 );
    CHECK_EQUAL( ftell( file ), 17 );
    }
    {
    ...
    int n;
    CHECK_EQUAL( fscanf( file, "%hx%n", &i, &n ), 1 );
    CHECK_FEQUAL( i, -32767, unsigned short, "%hu" ); // FIXME: This reports as 129... huh?!?
    CHECK_EQUAL( n, 7 );
    CHECK_EQUAL( ftell( file ), 35 );
    }
    {
